...
- **Título de panel** (800, ancho 118, 1.75rem en el teléfono y 2.1rem desde sm, 1.05, -0.025em): el título de cada pantalla de `/admin`, con raya doble de 5px debajo como los del home, pero a escala fija: en una herramienta el título no crece con la ventana. Va sin bajada: lo que sigue al título es directamente la lista o el formulario. Los títulos internos («Agregar profesional», «Días que no se atiende») van en el mismo ancho a 1.3rem.
...
El panel es una columna de 48rem (1rem de margen en el teléfono, 1.5rem desde sm) debajo del mismo encabezado del sitio, que en `/admin` pierde el menú y «Sacar turno». Arriba va el menú de dos filas; cada pantalla abre con su título y raya doble, sin bajada, y sigue directamente con listas, sin raya propia arriba —la raya doble del título ya las abre— y separadas con renglones de 1px. Las rayas separan una ficha de la otra y nunca cierran: cada renglón lleva la suya arriba, el primero no, y lo que sigue a la lista (lo dado de baja, el alta) trae la propia. Con rayas abajo se juntaban tres en pocos centímetros. Los formularios de alta van al final, detrás de otra raya de 2px y 3.5rem de aire. Los campos cortos van de a dos desde sm; en Agenda, el día y las dos horas de una franja van de a cuatro.
...
- **Enlace de texto:** para volver atrás sin peso («Cambiar de profesional», «Cambiar día y hora», «Cambiar»): texto en tinta suave a 0.95rem subrayado en color renglón a 4px de distancia, que pasa a tinta al hover. No es un botón y no lleva borde. En el panel es también lo que abre una acción que se confirma («Cancelar turno», «Quitar», «Dar de baja», «Ocultar de la página»), lo que se deshace («Volver a activar») y «Salir», con dos ajustes: va en 500 y subrayado en su mismo color, porque el subrayado en color renglón queda a 1.5:1 sobre la cartulina y ahí es lo único que dice «esto se toca»; y mide 44px de alto. Los que sacan algo, y «Salir», pasan a rojo de aviso al pasar el mouse o con el foco; en reposo son iguales a los demás.
...
- **Error:** el borde pasa a rojo de aviso y debajo aparece el texto del error en 0.95rem 700 en rojo, dentro de una región `aria-live` que ya existía vacía. El color nunca va solo: siempre hay texto. Es el mismo campo en todo el sitio, reserva y panel (ver «Errores y mensajes»).
...
- **Próximos:** todos los turnos desde hoy, en una sola lista partida por día. Arriba, la búsqueda (un campo con su rótulo y «Buscar» en contorno de tinta; es un formulario GET, así cada búsqueda es una URL), el filtro por profesional y el índice de días: las fichas de día de la reserva («Jue 24» en versalita, «2 turnos» debajo) con el rótulo «Ir al día», que llevan a cada día de la lista; con búsqueda no aparece. Cada día abre con su título a 1.3rem y una raya de 2px arriba, menos el primero; hoy y mañana se dicen en la frase («Hoy lunes, 21 de septiembre»), no con una etiqueta. Las filas son las de Hoy, con dos cambios: «Marcar atendido» y «No vino» sólo en los turnos de hoy, y la confirmación nombra el día («¿Cancelar el turno del jueves, 24 de septiembre a las 11:30?»).
- **Ficha de una lista** (profesional, sede, precio, especialidad): nombre en title a la izquierda, su baja o reactivación como enlace de texto a la derecha, y la letra chica debajo. Lo dado de baja no se mezcla con lo activo: va plegado al final de la lista. Los campos van debajo, y cada ficha guarda sola, con su resultado al lado del botón.
- **Lo dado de baja:** nunca se borra, porque los turnos pasados lo referencian, pero tampoco estorba. Al final de cada lista (profesionales, sedes, especialidades, precios) va un `<details>` cerrado, con la misma cruz en sello que las preguntas frecuentes, que dice «De baja (2)» u «Ocultos (1)». Adentro, un renglón por cada uno: el nombre en 700, un dato en tinta suave para reconocerlo y «Volver a activar» o «Mostrar en la página». Sin campos: no hay nada que editar en algo que no está en uso. Si no hay nada dado de baja, no aparece.
- **Confirmación en línea:** lo que no tiene vuelta atrás se pide dos veces sin modal. El enlace que la abre pasa a rojo de aviso al pasar el mouse o con el foco. El primer toque cambia el enlace por la pregunta en 700 («¿Cancelar el turno de las 19:00?»), el contorno de aviso chico («Sí, cancelar») y un «No» de texto. La confirmación toma el foco; Escape y «No» vuelven atrás y devuelven el foco.
- **Resultado de una acción:** lo que sale bien se confirma con un toast de Sonner abajo al centro, cerca del botón que se tocó, que nombra lo que cambió («Sede Centro guardada.», «Turno de las 16:00 cancelado.»). Lleva el estilo por defecto de Sonner, a propósito: es la única pieza del panel que no sigue este documento (esquinas redondeadas, sombra). Sólo se ajusta la letra, Archivo a 1rem en lugar de los 13px del sistema; dura seis segundos y tiene cruz para cerrar. Mientras guarda, el botón dice «Guardando…». Los errores no van en el toast: siguen las reglas de «Errores y mensajes».
...
### Errores y mensajes
Cada formulario del sitio —la reserva, cancelar, el ingreso y todos los del panel— valida con un schema de Zod que usan el navegador y el servidor, así que el mensaje es el mismo venga de donde venga. El comportamiento y el lugar de cada error son los mismos en todos.
- **Cuándo:** el error de un campo aparece al salir de él, no mientras se escribe; una vez marcado, se revalida en cada tecla y se va apenas se corrige. Al mandar se valida todo: si algo falla no se manda y el foco va al primer campo con error. Sin globitos del navegador (`noValidate`).
- **Error de un campo:** debajo del campo, con el borde en rojo de aviso. Lo que viene del servidor y es de un campo (un horario que se ocupó, una franja que se pisa con otra, una especialidad repetida) va en ese campo, no en un aviso.
- **Error del formulario:** lo que no es de ningún campo (la red, la base, una contraseña que no coincide) va en un aviso de error arriba del botón, con `role="alert"`. El mismo aviso en la reserva, al cancelar, al entrar y en el panel.
- **Error de una acción de un botón** (marcar atendido, dar de baja, quitar): en letra roja 700 a 0.95rem al lado del botón. Un aviso entero sería desproporcionado para un renglón.
- **Éxito:** en el panel, un toast que dice qué cambió. Nunca un error: un error no puede desaparecer solo a los cuatro segundos.
- **Cómo se escribe:** el error de un campo dice qué hacer, en voseo y sin punto final («Ingresá el apellido», «Elegí la sede», «Ingresá un DNI válido»). El del formulario es una oración completa que dice qué pasó y qué hacer, con punto («No pudimos guardar la sede. Probá de nuevo.»). El sitio le habla al paciente de «tu»; el panel habla del paciente en tercera persona («Ingresá el nombre»). Los textos que se repiten viven en `lib/forms.ts`.
...
- **Do** poner cada error donde se arregla: el de un campo en el campo, el del formulario en un aviso arriba del botón, y escribirlo diciendo qué hacer.
